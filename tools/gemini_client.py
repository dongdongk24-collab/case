...
📌 출처 및 링크 원칙 (반드시 준수):
- 복지로 서비스 세부페이지 URL 패턴 (반드시 이 형식 사용):
  https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/retrieveWlfareInfo.do?searchTerm=서비스명
  예시: 주거급여 → https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/retrieveWlfareInfo.do?searchTerm=주거급여
  예시: 기초생활보장 → https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/retrieveWlfareInfo.do?searchTerm=기초생활보장
- 광진구청 복지 서비스: https://www.gwangjin.go.kr/portal/bbs/B0000002/list.do?menuNo=200126
- 서울시 복지포털: https://welfare.seoul.go.kr
- 웹 검색으로 정확한 세부 URL 반드시 확인 후 제공
- 정확한 URL 모를 경우 복지로 검색 URL 사용 (메인 페이지 X)
